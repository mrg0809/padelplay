...
async def get_upcoming_player_events(current_user: dict = Depends(get_current_user)):
    ...
    supabase
    .from_("matches")
    .select("id, match_date, match_time, clubs (name)")
    ...
    if not matches_response.data:
        raise HTTPException(status_code=500, detail=matches_response.error)
    ...
    supabase
    .from_("lessons")
    .select("id, lesson_date, lesson_time, clubs (name)")
    ...
    if not lessons_response.data:
        raise HTTPException(status_code=500, detail=lessons_response.error)
    ...
    all_events = sorted(upcoming_matches + upcoming_lessons, key=lambda x: (x.match_date if isinstance(x, Match) else x.lesson_date, x.match_time if isinstance(x, Match) else x.lesson_time))
